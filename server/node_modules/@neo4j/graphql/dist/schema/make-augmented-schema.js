...
const Neo4jGraphQLSubscriptionsCDCEngine_1 = require("../classes/subscription/Neo4jGraphQLSubscriptionsCDCEngine");
...
function makeAugmentedSchema({ document, features, userCustomResolvers, subgraph, schemaModel, }) {
    ...
    for (const [typeName, deprecatedFields] of deprecation_map_1.bookmarkDeprecationMap) {
        const typeComposer = composer.getOTC(typeName);
        if ((0, utils_2.shouldAddDeprecatedFields)(features, "bookmark")) {
    ...
    if (features?.subscriptions && nodes.length) {
        const isCDCEngine = features.subscriptions instanceof Neo4jGraphQLSubscriptionsCDCEngine_1.Neo4jGraphQLSubscriptionsCDCEngine;
