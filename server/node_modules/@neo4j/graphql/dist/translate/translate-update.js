...
async function translateUpdate({ node, context, }) {
    ...
    const relationshipValidationStr = (0, create_relationship_validation_string_1.createRelationshipValidationString)({ node, context, varName });
    const updateQuery = new cypher_builder_1.default.Raw((env) => {
        const cypher = [
            ...
            ...(context.subscriptionsEnabled ? [`,\ncollect(DISTINCT m) as ${constants_1.META_CYPHER_VARIABLE}`] : []),
        ]
            .filter(Boolean)
            .join("\n");
        return [
